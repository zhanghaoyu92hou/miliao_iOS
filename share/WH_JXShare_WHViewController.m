...
@interface WH_JXShare_WHViewController () <UITextViewDelegate>

@end

@implementation WH_JXShare_WHViewController
...
- (void)viewDidLoad{
    ...
    self.wh_textView = [[UITextView alloc] initWithFrame:CGRectMake(20, 10, JX_SCREEN_WIDTH-40, 100)];
    self.wh_textView.delegate = self;
    self.wh_textView.textColor = [UIColor lightGrayColor];
    self.wh_textView.font = sysFontWithSize(17);
    self.wh_textView.text = @"这一刻的想法..";
    [bigView addSubview:self.wh_textView];
    
    
    CGFloat b = self.wh_image.size.height/self.wh_image.size.width;
    UIImageView *imageV = [[UIImageView alloc] initWithFrame:CGRectMake(20, CGRectGetMaxY(self.wh_textView.frame)+20, JX_SCREEN_WIDTH/5, JX_SCREEN_WIDTH/5*b)];
    imageV.image = self.wh_image;
...
- (void)onSend {
    if (self.wh_delegate && [self.wh_delegate respondsToSelector:@selector(WH_sendToLifeCircleSuccesWH_sendToLifeCircleSucces:)]) {
        [self.wh_delegate WH_sendToLifeCircleSucces:self];
