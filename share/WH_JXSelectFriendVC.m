...
@interface WH_JXSelectFriendVC () <UITextFieldDelegate,UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation WH_JXSelectFriendVC
...
- (void)getData {
    self.data = [[WH_JXShareUser shareInstance] WH_getAllUser];
}
...
- (void)textFieldDidChange:(UITextField *)textField {
    self.data = [[WH_JXShareUser shareInstance] WH_fetchSearchUserWithString:textField.text];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellIdentifier = @"WH_JXFriend_WHCell";
    WH_JXFriend_WHCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [[WH_JXFriend_WHCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
    }
    
    WH_JXShareUser *user = self.data[indexPath.row];
    [cell WH_setDataWithUser:user];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.wh_delegate && [self.wh_delegate respondsToSelector:@selector(sendToFriendSuccess:user:)]) {
        [self.wh_delegate sendToFriendSuccess:self user:user];
