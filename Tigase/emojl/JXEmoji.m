...
@interface JXEmoji () <MiXin_JXActionSheet_MiXinVCDelegate>

@property (nonatomic, strong) MiXin_JXActionSheet_MiXinVC *actionVC;
...
- (void)setText:(NSString *)text {
    ...
    for (int i=0;i<[data count];i++) {
         ...
         if ([str hasPrefix:BEGIN_FLAG]&&[str hasSuffix:END_FLAG]) {
              ...
              if(isFace){
                  if (n < g_constant.emojiArray.count) {
                      NSDictionary *dic = [g_constant.emojiArray objectAtIndex:n];
                      // 创建图片图片附件
    ...
    for (NSTextCheckingResult *match in self.matches) {
        ...
        if ([match resultType] == NSTextCheckingTypePhoneNumber) {
            ...
            if ([self isIndex:charIndex inRange:matchRange]) {
                ...
                self.actionVC = [[MiXin_JXActionSheet_MiXinVC alloc] initWithImages:@[] names:@[Localized(@"JX_Copy"),Localized(@"JXEmoji_CallPhone")]];
...
//                self.actionVC = [[MiXin_JXActionSheet_MiXinVC alloc] initWithImages:@[] names:@[Localized(@"JX_Copy"),Localized(@"JXEmoji_OpenUrl")]];
//                self.actionVC.delegate = self;
//                self.actionVC.tag = 2;
//                [g_App.window addSubview:self.actionVC.view];
...
//                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    ...
                    MiXin_webpage_MiXinVC *webVC = [MiXin_webpage_MiXinVC alloc];
