...
@implementation WH_DepartObject
...
-(void)setDepartes:(NSArray *)departes{
    if (departes != nil && departes.count > 0) {
        if ([departes[0] isKindOfClass:[WH_DepartObject class]]) {
            _departes = departes;
            NSMutableArray * childArr = [NSMutableArray array];
        ...
        for (NSDictionary * childDict in departes) {
            WH_DepartObject * departObj = [WH_DepartObject WH_departmentObjectWith:childDict allData:_allDict];
...
-(void)setEmployees:(NSArray *)employees{
    if (employees != nil && employees.count > 0) {
        if ([employees[0] isKindOfClass:[WH_EmployeObject class]]) {
        ...
        for (NSDictionary * childDict in employees) {
            WH_EmployeObject * employeObj = [WH_EmployeObject employWithDict:childDict];
...
-(void)setChildren:(NSArray *)children{
    if ([children[0] isKindOfClass:[WH_DepartObject class]] || [children[0] isKindOfClass:[WH_EmployeObject class]]) {
        _children = children;
        return;
    }
//    NSMutableArray *childArray = [NSMutableArray array];
...
- (void)WH_addChild:(id)child
{
    ...
    if ([child isMemberOfClass:[WH_DepartObject class]]) {
    ...
    }else if ([child isMemberOfClass:[WH_EmployeObject class]]) {
