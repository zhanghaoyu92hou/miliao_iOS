...
-(void)setDepartes:(NSArray *)departes{
    if (departes != nil && departes.count > 0) {
        ...
        for (NSDictionary * childDict in departes) {
            DepartObject * departObj = [DepartObject MiXin_departmentObjectWith:childDict allData:_allDict];
...
- (void)MiXin_removeChild:(id)child
{
    if ([child isMemberOfClass:[DepartObject class]]) {
        NSMutableArray *departs = [self.departes mutableCopy];
...
/** 创建部门节点 */
+(instancetype)MiXin_departmentObjectWith:(NSDictionary *)nodeDict allData:(NSMutableArray *)allDict{
