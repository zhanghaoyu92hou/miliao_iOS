//
//  MiXin_OrganizTree_MiXinViewController.m
//  shiku_im
//
...
#import "MiXin_Employee_MiXinTableViewCell.h"
#import "MiXin_JX_DownListView.h"

#import "MiXin_JXAddDepart_MiXinViewController.h"
#import "MiXin_JXSelFriend_MinXinVC.h"
#import "MiXin_JXSelectFriends_MinXinVC.h"
#import "MiXin_JXUserInfo_MiXinVC.h"
#import "MiXin_JXSelDepart_MiXinViewController.h"

@interface MiXin_OrganizTree_MiXinViewController ()<RATreeViewDelegate, RATreeViewDataSource,AddDepartDelegate,SelDepartDelegate,UIAlertViewDelegate>
...
@implementation MiXin_OrganizTree_MiXinViewController
- (id)init
{
    self = [super init];
    if (self) {
...
- (void)viewDidLoad{
    ...
    _moreButton = [UIFactory MiXin_create_MiXinButtonWithImage:@"im_003_more_button_normal" highlight:nil target:self selector:@selector(onMore:)];
...
-(void)MiXin_create_MiXinTreeView{
    ...
    [treeView registerClass:[MiXin_Employee_MiXinTableViewCell class] forCellReuseIdentifier:NSStringFromClass([MiXin_Employee_MiXinTableViewCell class])];
...
- (void)treeView:(RATreeView *)treeView willExpandRowForItem:(id)item
{
    if ([item isMemberOfClass:[DepartObject class]]) {
        MiXin_Organiz_MiXinTableViewCell * cell = (MiXin_Organiz_MiXinTableViewCell *)[self.treeView cellForItem:item];
...
-(void)treeView:(RATreeView *)treeView didSelectRowForItem:(id)item{
    if ([item isMemberOfClass:[EmployeObject class]]){
        [self MiXin_showEmployeeDownListView:item];
    }else{
        DepartObject * depart = item;
...
{
//    MiXin_Organiz_MiXinObject *dataObject = item;
    NSInteger level = [self.treeView levelForCellForItem:item];
...
//    BOOL expanded = [self.treeView isCellForItemExpanded:item];
    ...
    if ([item isMemberOfClass:[DepartObject class]]) {
        ...
        MiXin_Employee_MiXinTableViewCell * cell = [self.treeView dequeueReusableCellWithIdentifier:NSStringFromClass([MiXin_Employee_MiXinTableViewCell class])];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
-(void)showDepartDownListView:(DepartObject *)orgObject{
    ...
    MiXin_Organiz_MiXinTableViewCell * cell = (MiXin_Organiz_MiXinTableViewCell *)[self.treeView cellForItem:orgObject];
...
//        downListView.listContents = @[@"加部门",@"加员工",@"改部门名",@"删除"];
    ...
    [downListView MiXin_downlistPopOption:^(NSInteger index, NSString *content) {
        ...
        if (level == 0) {
            if (index == 0) {
                [weakSelf MiXin_addDepartWithParent:orgObject];
            }else if (index == 1) {
            ...
            switch (index) {
                ...
                case 1:{
                    [weakSelf MiXin_chooseEmployeeWithParent:orgObject];
                    break;
                }
                case 2:{
                    [weakSelf MiXin_changeDepartNameWithParent:orgObject];
                    break;
                }
                case 3:{
...
-(void)MiXin_showEmployeeDownListView:(EmployeObject *)empObject{
    _currentOrgObj = empObject;
//    NSInteger level = [self.treeView levelForCellForItem:empObject];
    MiXin_Employee_MiXinTableViewCell * cell = (MiXin_Employee_MiXinTableViewCell *)[self.treeView cellForItem:empObject];
...
//        
//        UILabel * employeeLabel = [UIFactory MiXin_create_MiXinLabelWith:CGRectMake(0, 0, CGRectGetWidth(employeeView.frame), CGRectGetHeight(employeeView.frame)) text:@"+员工" font:g_factory.font17 textColor:[UIColor blackColor] backgroundColor:nil];
...
//    }
//    
//    UILabel * departLabel = [UIFactory MiXin_create_MiXinLabelWith:CGRectMake(0, 0, CGRectGetWidth(departView.frame), CGRectGetHeight(departView.frame)) text:@"+部门" font:g_factory.font17 textColor:[UIColor blackColor] backgroundColor:nil];
...
//        case 11:{
//            //新员工
//            MiXin_JXSelFriend_MinXinVC * addEmployeeVC = [[MiXin_JXSelFriend_MinXinVC alloc] init];
...
/** 加部门 */
-(void)MiXin_addDepartWithParent:(DepartObject *)orgObject{
    ...
    self.rowActionAfterRequestBlock = ^(id sender) {
        ...
        DepartObject * childDepart = [DepartObject MiXin_departmentObjectWith:departData allData:nil];
        
        [orgObject MiXin_addChild:childDepart];
...
/** 改部门名 */
-(void)MiXin_changeDepartNameWithParent:(DepartObject *)orgObject{
...
/** 加员工 */
-(void)MiXin_chooseEmployeeWithParent:(DepartObject *)orgObject{
    ...
    MiXin_JXSelectFriends_MinXinVC * addEmployeeVC = [[MiXin_JXSelectFriends_MinXinVC alloc] init];
    addEmployeeVC.delegate = self;
...
//            index = [parent.children indexOfObject:self.item];
            ...
            [parent MiXin_removeChild:weakSelf.item];
...
-(void)inputViewController:(OrganizAddType) type oldName:(NSString *)oldName{
    MiXin_JXAddDepart_MiXinViewController * addDepartVC = [[MiXin_JXAddDepart_MiXinViewController alloc] init];
...
//    NSDictionary * empDepart = _allDataDict[parentId];
    ...
    MiXin_JXSelDepart_MiXinViewController * selDepartVC = [[MiXin_JXSelDepart_MiXinViewController alloc] init];
...
#pragma mark 加员工
-(void)addEmployeeDelegate:(MiXin_JXSelectFriends_MinXinVC*)vc{
    NSArray * allArr;
...
-(void)selNewDepartmentWith:(DepartObject *)newDepart{
    ...
    MiXin_Organiz_MiXinTableViewCell * cell = (MiXin_Organiz_MiXinTableViewCell *)[self.treeView cellForItem:newDepart];
    if (!cell.arrowExpand) {
        cell.arrowExpand = YES;
...
//        [weakSelf.treeView beginUpdates];
        [newDepart MiXin_addChild:employeeOBJ];
        [oldDepart MiXin_removeChild:employeeOBJ];
...
-(NSArray <DepartObject *>*) constructDepartObject:(NSArray *)departArray{
    ...
    for (NSDictionary * rootData in rootArr) {
        DepartObject * departObj  = [DepartObject MiXin_departmentObjectWith:rootData allData:allDataArr];
...
//    NSMutableArray * departArr = [[NSMutableArray alloc] init];
//    for (NSDictionary * rootData in rootArr) {
//        DepartObject * departObj  = [DepartObject MiXin_departmentObjectWith:rootData allData:_allDataDict];
//        [departArr addObject:departObj];
//    }
