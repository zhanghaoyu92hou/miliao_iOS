...
@interface WH_JXLinksShare_WHVC ()
...
@implementation WH_JXLinksShare_WHVC
...
- (void)WH_setupViews {
    ...
    int btnX = leftInset;
    int btnY = 0;
    button = [self WH_create_WHButtonWithImage:@"im_linksShare_send_friend" highlight:@"im_linksShare_send_friend" target:self.delegate selector:self.onSend title:Localized(@"JXSendToFriend")];
    ...
    button = [self WH_create_WHButtonWithImage:@"im_linksShare_life" highlight:@"im_linksShare_life" target:self.delegate selector:self.onShare title:Localized(@"JX_ShareLifeCircle")];
    button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
    [topScrollV addSubview:button];
    
    // 收藏
    btnX += (button.frame.size.width +leftInset);
    button = [self WH_create_WHButtonWithImage:@"im_linksShare_collection" highlight:@"im_linksShare_collection" target:self.delegate selector:self.onCollection title:Localized(@"JX_Collection")];
    button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
    [topScrollV addSubview:button];
    ...
    if ([g_default boolForKey:WH_ThirdPartyLogins]) {
        // 发送给微信好友
        btnX += (button.frame.size.width +leftInset);
        button = [self WH_create_WHButtonWithImage:@"im_linksShare_send_friend_WX" highlight:@"im_linksShare_send_friend_WX" target:self.delegate selector:self.onWXSend title:Localized(@"JXSendToWXFriend")];
        button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
        [topScrollV addSubview:button];
        
        // 分享到微信朋友圈
        btnX += (button.frame.size.width +leftInset);
        button = [self WH_create_WHButtonWithImage:@"im_linksShare_life_WX" highlight:@"im_linksShare_life_WX" target:self.delegate selector:self.onWXShare title:Localized(@"JX_ShareLifeWXCircle")];
        button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
        [topScrollV addSubview:button];
    ...
    // 在Safari中打开
    btnX += (button.frame.size.width +leftInset);
    button = [self WH_create_WHButtonWithImage:@"im_linksShare_safari" highlight:@"im_linksShare_safari" target:self.delegate selector:self.onSafari title:Localized(@"JX_OpenInSafari")];
    button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
    [topScrollV addSubview:button];
    ...
    // 浮窗
        btnX = leftInset;
        btnY = 0;
    //    button = [self WH_create_WHButtonWithImage:self.isFloatWindow ? @"im_linksShare_un_float" : @"im_linksShare_float" highlight:self.isFloatWindow ? @"im_linksShare_un_float" : @"im_linksShare_float" target:self.delegate selector:self.onFloatWindow title:self.isFloatWindow ? [NSString stringWithFormat:@"%@%@",Localized(@"JX_Close"),Localized(@"JX_FloatingWindow")] : Localized(@"JX_FloatingWindow")];
    //    button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
    //    [botScrollV addSubview:button];
    //
    //    // 投诉
    //    btnX += (button.frame.size.width +leftInset);
        button = [self WH_create_WHButtonWithImage:@"im_linksShare_complaint" highlight:@"im_linksShare_complaint" target:self.delegate selector:self.onReport title:Localized(@"UserInfoVC_Complaint")];
        button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
        [botScrollV addSubview:button];
    
    // 复制链接
    btnX += (button.frame.size.width +leftInset);
    button = [self WH_create_WHButtonWithImage:@"im_linksShare_link" highlight:@"im_linksShare_link" target:self.delegate selector:self.onPasteboard title:[NSString stringWithFormat:@"%@%@",Localized(@"JX_Copy"),Localized(@"JXLink")]];
    button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
    [botScrollV addSubview:button];
    // 刷新
    btnX += (button.frame.size.width +leftInset);
    button = [self WH_create_WHButtonWithImage:@"im_linksShare_update" highlight:@"im_linksShare_update" target:self.delegate selector:self.onUpdate title:Localized(@"JX_Refresh")];
    button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
    [botScrollV addSubview:button];
    // 搜索页面内容
    btnX += (button.frame.size.width +leftInset);
    button = [self WH_create_WHButtonWithImage:@"im_linksShare_search" highlight:@"im_linksShare_search" target:self.delegate selector:self.onSearch title:Localized(@"JX_SearchPageContent")];
    button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
    [botScrollV addSubview:button];
    
    // 调整字体
    btnX += (button.frame.size.width +leftInset);
    button = [self WH_create_WHButtonWithImage:@"im_linksShare_type" highlight:@"im_linksShare_type" target:self.delegate selector:self.onTextType title:Localized(@"JX_AdjustTheFont")];
    button.frame = CGRectMake(btnX, btnY, SHARE_HEIGHT, SELECTIMAGE_WIDTH);
    [botScrollV addSubview:button];
                            ...
                            title:(NSString*)title
{
    ...
    CGSize size = [title boundingRectWithSize:CGSizeMake(SHARE_HEIGHT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:sysFontWithSize(13)} context:nil].size;
