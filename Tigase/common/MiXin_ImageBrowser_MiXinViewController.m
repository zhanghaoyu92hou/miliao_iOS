//
//  MiXin_ImageBrowser_MiXinViewController.m
//  ImageBrowser
//
...
#import "MiXin_ImageBrowser_MiXinViewController.h"
#import "PhotoView.h"
#import "MiXin_JXUserInfo_MiXinVC.h"
#import "MiXin_webpage_MiXinVC.h"
#import "MiXin_JXActionSheet_MiXinVC.h"
#import "KKImageEditorViewController.h"

@interface MiXin_ImageBrowser_MiXinViewController ()<UIScrollViewDelegate,PhotoViewDelegate,MiXin_JXActionSheet_MiXinVCDelegate,KKImageEditorDelegate,UINavigationControllerDelegate>{
...
@end
static MiXin_ImageBrowser_MiXinViewController *shared;
@implementation MiXin_ImageBrowser_MiXinViewController

//+ (MiXin_ImageBrowser_MiXinViewController*)sharedInstance{
//    static dispatch_once_t onceToken;
//    dispatch_once(&onceToken, ^{
//        shared = [[MiXin_ImageBrowser_MiXinViewController alloc]init];
//    });
//    return shared;
//}
- (void)dealloc{
...
+(void)show:(UIViewController *)handleVC delegate:(id)delegate type:(PhotoBroswerVCType)type contentArray:(NSMutableArray *)contentArray index:(NSUInteger)index imagesBlock:(NSArray *(^)())imagesBlock{
    ...
    MiXin_ImageBrowser_MiXinViewController *imgBrowserVC = [[MiXin_ImageBrowser_MiXinViewController alloc] init];
...
-(void)hideScanImageVC{
    if (self.delegate && [self.delegate respondsToSelector:@selector(MiXin_dismissImageBrowserVC)]) {
        [self.delegate MiXin_dismissImageBrowserVC];
    }
...
- (void)setupActionSheet {
    MiXin_JXActionSheet_MiXinVC *actionVC = [[MiXin_JXActionSheet_MiXinVC alloc] initWithImages:@[] names:@[Localized(@"JX_IdentifyTheQrCode"),Localized(@"JX_ImageEditTitle"),Localized(@"ImageBrowser_save")]];
    actionVC.delegate = self;
    [self presentViewController:actionVC animated:NO completion:nil];
}

- (void)actionSheet:(MiXin_JXActionSheet_MiXinVC *)actionSheet didButtonWithIndex:(NSInteger)index {
    if (index == 0) {
...
- (void) distinguishQRCode {
    ...
    if(tempImageView.image){
        ...
        if ([self.delegate respondsToSelector:@selector(MiXin_imageBrowserVCQRCodeAction:)]) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self hideScanImageVC];
                [self.delegate MiXin_imageBrowserVCQRCodeAction:scannedResult];
            });
        }
