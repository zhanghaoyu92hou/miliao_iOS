...
#define WH_GDataXMLNode_DEFINE_GLOBALS 1
...
// WH_GDataXMLNode does not require pulling in all of GData.
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
  ...
  NSString *localName = [WH_GDataXMLNode localNameForName:name];
...
@interface WH_GDataXMLNode (PrivateMethods)
...
- (WH_GDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
@implementation WH_GDataXMLNode
...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
  ...
  if (theXMLNode->type == XML_ELEMENT_NODE) {
    ...
    theClass = [WH_GDataXMLNode class];
  }
...
#if DEBUG
  NSCAssert(chars != NULL, @"WH_GDataXMLNode sees an unexpected empty string");
#endif
...
- (NSArray *)children {
  ...
  if (xmlNode_ != NULL) {
    ...
    while (currChild != NULL) {
      WH_GDataXMLNode *node = [WH_GDataXMLNode nodeBorrowingXMLNode:currChild];
...
- (WH_GDataXMLNode *)childAtIndex:(unsigned)index {
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        for (NSString *prefix in namespaces) {
          ...
          if (result != 0) {
#if DEBUG
            NSCAssert1(result == 0, @"WH_GDataXMLNode XPath namespace %@ issue",
                      prefix);
#endif
          }
        }
        ...
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
            ...
            if (result != 0) {
#if DEBUG
              NSCAssert1(result == 0, @"WH_GDataXMLNode XPath namespace %s issue",
                        prefix);
#endif
            }
          }
      ...
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (int index = 0; index < nodeSet->nodeNr; index++) {
            ...
            WH_GDataXMLNode *node = [WH_GDataXMLNode nodeBorrowingXMLNode:currNode];
            if (node) {
              [array addObject:node];
            }
          }
...
- (BOOL)isEqual:(WH_GDataXMLNode *)other {
  if (self == other) return YES;
  if (![other isKindOfClass:[WH_GDataXMLNode class]]) return NO;
...
- (NSUInteger)hash {
  return (NSUInteger) (__bridge void *) [WH_GDataXMLNode class];
}
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
        WH_GDataXMLNode *node = [WH_GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)addNamespace:(WH_GDataXMLNode *)aNamespace {
...
- (void)WH_addChild:(WH_GDataXMLNode *)child {
...
- (void)WH_removeChild:(WH_GDataXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    for (WH_GDataXMLNode *child in children) {
...
- (NSArray *)attributes {
  ...
  if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
    ...
    while (prop != NULL) {
      ...
      WH_GDataXMLNode *node = [WH_GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
      if (array == nil) {
        array = [NSMutableArray arrayWithObject:node];
      } else {
        [array addObject:node];
...
- (void)addAttribute:(WH_GDataXMLNode *)attribute {
...
- (WH_GDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
  // search the cached attributes list for the WH_GDataXMLNode with
  // the underlying xmlAttrPtr
  NSArray *attributes = [self attributes];

  for (WH_GDataXMLNode *attr in attributes) {
...
- (WH_GDataXMLNode *)attributeForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr) {
      WH_GDataXMLNode *attr = [self attributeForXMLNode:attrPtr];
      return attr;
    }
  }
  return nil;
...
- (WH_GDataXMLNode *)attributeForLocalName:(NSString *)localName
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
  if (xmlDoc_ != NULL) {
    WH_GDataXMLNode *docNode = [GDataXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];
