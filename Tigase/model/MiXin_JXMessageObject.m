//
//  MiXin_JXMessageObject.m
//
//  Created by Reese on 13-8-11.
//  Copyright (c) 2013年 Reese. All rights reserved.
//

#import "MiXin_JXMessageObject.h"
#import "FMDatabase.h"
#import "FMResultSet.h"
#import "NSData+XMPP.h"
#import "XMPPStream.h"
#import "MiXin_JXFriendObject.h"
#import "MiXin_JXBlogObject.h"
#import "versionManage.h"
//#import "ChatCacheFileUtil.h"
#import "FileInfo.h"
#import "MiXin_JXRoomRemind.h"
#import "MiXin_GDataXMLNode.h"
#ifdef Live_Version
#import "MiXin_JXLiveJid_MiXinManager.h"
#endif


#import "MiXin_JXBaseChat_MiXinCell.h"
#import "MiXin_JXMessage_MiXinCell.h"
#import "MiXin_JXImage_MiXinCell.h"
#import "MiXin_JXFile_MiXinCell.h"
#import "MiXin_JXVideo_MiXinCell.h"
#import "MiXin_JXAudio_MiXinCell.h"
#import "MiXin_JXLocation_MiXinCell.h"
#import "MiXin_JXCard_MiXinCell.h"
#import "MiXin_JXRedPacket_MiXinCell.h"
#import "MiXin_JXRemind_MiXinCell.h"
#import "MiXin_JXGif_MiXinCell.h"
#import "MiXin_JXSystemImage1_MiXinCell.h"
#import "MiXin_JXSystemImage2_MiXinCell.h"
#import "MiXin_JXAVCall_MiXinCell.h"
#import "MiXin_JXLink_MiXinCell.h"
#import "MiXin_JXShake_MiXinCell.h"
#import "MiXin_JXMergeRelay_MiXinCell.h"
#import "MiXin_JXShare_MiXinCell.h"
#import "MiXin_JXTransfer_MiXinCell.h"
#import "MiXin_JXReply_MiXinCell.h"


@implementation MiXin_JXMessageObject
...
static MiXin_JXMessageObject *shared;

+(MiXin_JXMessageObject*)sharedInstance{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shared=[[MiXin_JXMessageObject alloc]init];
    });
    return shared;
}
...
-(void)dealloc{
    NSLog(@"MiXin_JXMessageObject.dealloc");
...
//    self.isReadDel = [NSNumber numberWithBool:[rs boolForColumn:kMESSAGE_isReadDel]];
    self.isReadDel = [NSNumber numberWithInt:[rs intForColumn:kMESSAGE_isReadDel]];
...
-(BOOL)insert:(NSString*)room
{
    ...
    if(self.isMySend){//发送
        ...
        MiXin_JXUserObject *user = [[MiXin_JXUserObject sharedInstance] getUserById:self.toUserId];
...
-(NSString*)getLastContent{
    ...
    switch (n) {
        ...
        case XMPP_TYPE_PASS:
            if(self.isMySend)
                s = Localized(@"MiXin_JXFriendObject_Passed");
            else
                s = Localized(@"MiXin_JXFriendObject_PassGo");
            break;
        ...
        case XMPP_TYPE_NEWSEE:
            if(self.isMySend)
                s = Localized(@"MiXin_JXFriendObject_Followed");
            else
                s = Localized(@"MiXin_JXFriendObject_FollowYour");
            break;
        case XMPP_TYPE_DELSEE:
            if(self.isMySend)
                s = Localized(@"MiXin_JXFriendObject_CencalFollowed");
            else
                s = Localized(@"MiXin_JXFriendObject_CancelFollow");
            break;
        case XMPP_TYPE_RECOMMEND:
            s = Localized(@"MiXin_JXFriendObject_RecomYou");
        */
        default:
                s = self.content;
            break;
    ...
    if (self.isGroup && [self.type intValue] != kWCMessageTypeRemind) {
        ...
        MiXin_JXUserObject *allUser = [[MiXin_JXUserObject alloc] init];
...
-(NSMutableArray *)fetchMessageListWithUser:(NSString *)userId byAllNum:(NSInteger)num pageCount:(int)pageCount startTime:(NSDate *)startTime
{
    ...
    NSString *queryString=[NSString stringWithFormat:@"select * from msg_%@ where (fromUserId=? or toUserId=?) and type<>26 and timeSend >= ? and timeSend > ? order by timeSend desc,messageNo desc limit %ld,%d",userId,num,pageCount];
...
//    NSString *queryString=[NSString stringWithFormat:@"select * from msg_%@ order by timeSend desc,messageNo desc limit ?*%d,%d",userId,PAGE_SHOW_COUNT,PAGE_SHOW_COUNT];
    ...
    while ([rs next]) {
        MiXin_JXMessageObject *p=[[MiXin_JXMessageObject alloc] init];
        [p fromRs:rs];
...
- (CGFloat)getMsgChatHeight:(MiXin_JXMessageObject *)msg {
 
    switch ([msg.type intValue]) {
        case kWCMessageTypeText:
            return [MiXin_JXMessage_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeImage:
            return [MiXin_JXImage_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeVoice:
            return [MiXin_JXAudio_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeLocation:
            return [MiXin_JXLocation_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeGif:
            return [MiXin_JXGif_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeVideo:
            return [MiXin_JXVideo_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeAudio:
            return [MiXin_JXVideo_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeCard:
            return [MiXin_JXCard_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeFile:
            return [MiXin_JXFile_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeRemind:
            return [MiXin_JXRemind_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeRedPacket:
            return [MiXin_JXRedPacket_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeTransfer:
            return [MiXin_JXTransfer_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeSystemImage1:
            return [MiXin_JXSystemImage1_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeSystemImage2:
            return [MiXin_JXSystemImage2_MiXinCell getChatCellHeight:msg];
            break;
        ...
        case kWCMessageTypeVideoChatEnd:
            return [MiXin_JXAVCall_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeLink:
            return [MiXin_JXLink_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeShake:
            return [MiXin_JXShake_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeMergeRelay:
            return [MiXin_JXMergeRelay_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeShare:
            return [MiXin_JXShare_MiXinCell getChatCellHeight:msg];
            break;
        case kWCMessageTypeReply:
            return [MiXin_JXReply_MiXinCell getChatCellHeight:msg];
            break;
        default:
            return [MiXin_JXBaseChat_MiXinCell getChatCellHeight:msg];
            break;
    }
}
...
-(NSMutableArray *)fetchRecentChatByPage:(int)pageIndex
{
    ...
    while ([rs next]) {
        ...
        MiXin_JXMsgAndUserObject *unionObject=[MiXin_JXMsgAndUserObject unionWithMessage:p andUser:user ];
...
-(NSMutableArray *)fetchRecentChat
{
    ...
    queryString=[NSString stringWithFormat:@"select * from friend where (status=8 or status=2 or status=0 or status=10) and length(content)>0 and (topTime is null or topTime = 0)  and userId != %@ and timeSend > ifnull(clearChatTime,0) order by timeSend desc", FRIEND_CENTER_USERID];
...
// 搜索聊天记录
-(NSArray <MiXin_JXMessageObject *>*)fetchSearchMessageWithUserId:(NSString *)userId String:(NSString *)str {
    NSString* myUserId = MY_USER_ID;
    if([myUserId length]<=0)
        return nil;
    FMDatabase* db = [[JXXMPP sharedInstance] openUserDb:myUserId];
...
-(NSArray <MiXin_JXMessageObject *>*)fetchDelMessageWithUserId:(NSString *)userId {
    NSString* myUserId = MY_USER_ID;
    if([myUserId length]<=0)
        return nil;
    FMDatabase* db = [[JXXMPP sharedInstance] openUserDb:myUserId];
    NSString *queryString=[NSString stringWithFormat:@"select * from msg_%@ where isReadDel = 1 and fromUserId != %@ and timeSend > ? order by timeSend desc",userId,myUserId];
    FMResultSet *rs=[db executeQuery:queryString,[self clearChatDateWithUserId:userId db:db]];
...
-(void)doReceiveRoomRemind{
    if(![self isRoomControlMsg])
        return;
    MiXin_JXRoomRemind* p = [[MiXin_JXRoomRemind alloc] init];
    [p fromObject:self];
    //    [p insert];
    [p notify];
    //    [p release];
...
-(void)doReceiveNewBlog{
    ...
    if([self.type intValue] != XMPP_TYPE_NEWBLOG)
        return;
    MiXin_JXBlogObject* p = [[MiXin_JXBlogObject alloc] init];
    [p fromObject:self];
    [p insert];
    //    [p release];
}
...
-(BOOL)doNewRemindMsg{
    ...
    MiXin_JXMessageObject* msg = self;
//    bool是否是直播间
    BOOL isLiveMsg = NO;
#ifdef Live_Version
    isLiveMsg = [[MiXin_JXLiveJid_MiXinManager shareArray] contains:msg.objectId];
#endif
    NSString* s=nil;BOOL b=1;
    MiXin_JXUserObject* user = [[MiXin_JXUserObject sharedInstance] getUserById:msg.objectId];
    ...
    switch ([msg.type intValue]) {
        case kRoomRemind_RoomName:
            s = [NSString stringWithFormat:@"%@%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_UpdateRoomName"),msg.content];
            break;
        case kRoomRemind_NickName:{
            s = [NSString stringWithFormat:@"%@%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_UpdateNickName"),msg.content];
...
//            }
        ...
        case kRoomRemind_AddMember:
            if([msg.toUserId isEqualToString:msg.fromUserId])
                s = [NSString stringWithFormat:@"%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_GroupChat")];
            else
                s = [NSString stringWithFormat:@"%@%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_InterFriend"),msg.toUserName];
            break;
        ...
        case kRoomRemind_DelMember:
            if([msg.toUserId isEqualToString:msg.fromUserId]){
                if(isLiveMsg){
                    ...
                    s = [NSString stringWithFormat:@"%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_OutGroupChat")];
                }
            }else{
                if(isLiveMsg){
                    s = [NSString stringWithFormat:@"%@%@",msg.toUserName,Localized(@"JX_LiveVC_kickLive")];//被踢出
                    ...
                    if ([msg.toUserId isEqualToString:MY_USER_ID]) {
                        ...
                        if ([msg.fromUserName isEqualToString:msg.toUserName]) {
                            s = [NSString stringWithFormat:@"%@%@",msg.fromUserName,Localized(@"MiXin_JXRoomMember_MiXinVC_OutPutRoom")];
                        }else {
                            s = [NSString stringWithFormat:@"%@%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_KickOut"),msg.toUserName];
                        }
                    }
...
//                b = 0;
        ...
        case kRoomRemind_NewNotice:
        {
            ...
            s = [NSString stringWithFormat:@"%@%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_AddNewAdv"),newContent];
        }
        ...
        case kRoomRemind_DisableSay:{
            if([msg.content longLongValue]==0){
                s = [NSString stringWithFormat:@"%@%@%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_Yes"),msg.toUserName,Localized(@"MiXin_JXMessageObject_CancelGag")];
            }else{
                ...
                s = [NSString stringWithFormat:@"%@%@%@%@%@",msg.fromUserName,Localized(@"MiXin_JXMessageObject_Yes"),msg.toUserName,Localized(@"MiXin_JXMessageObject_SetGagWithTime"),t];
                d = nil;
            }
            break;
        }
        ...
        case kRoomRemind_SetManage:{
            if ([msg.content integerValue] == 1) {
                s = [NSString stringWithFormat:@"%@%@%@%@",Localized(@"JXGroup_Owner"),Localized(@"MiXin_JXSetting_MinXinVC_Set"),msg.toUserName,Localized(@"JXMessage_admin")];
            }else {
        ...
        case kRoomRemind_SetInvisible:{
            if ([msg.content integerValue] == 1) {
                s = [NSString stringWithFormat:@"%@%@%@%@",msg.fromUserName,Localized(@"MiXin_JXSetting_MinXinVC_Set"),msg.toUserName,Localized(@"JX_ForTheInvisibleMan")];
                ...
                s = [NSString stringWithFormat:@"%@%@%@%@",msg.fromUserName,Localized(@"MiXin_JXSetting_MinXinVC_Set"),msg.toUserName,@"为监控人"];
...
-(void)copy:(MiXin_JXMessageObject*)p{
...
+(void)msgWithFriendStatus:(NSString*)userId status:(NSInteger)status{
    MiXin_JXMessageObject *msg=[[MiXin_JXMessageObject alloc] init];
    ...
    switch (status) {
        case friend_status_black:
            if(isRoom)
                s = Localized(@"MiXin_JXMessageObject_ShieldMessage");
            else
                s = Localized(@"MiXin_JXMessageObject_AddBlack");
            break;
        case friend_status_none:
            s = Localized(@"MiXin_JXMessageObject_NotFriend");
            break;
        case friend_status_see:
            s = Localized(@"MiXin_JXMessageObject_FollowHim");
            break;
        case friend_status_friend:
            if(isRoom)
                s = Localized(@"MiXin_JXMessageObject_ReceMessage");
            else{
//                s = Localized(@"MiXin_JXFriendObject_StartChat");
//                n = kWCMessageTypeText;
                s = Localized(@"MiXin_JXMessageObject_BeFriendAndChat");
            }
            break;
    }
...
//-(void)downloadFile:(MiXin_JXImageView*)iv{
//    if([content length]<=0)
//        return;
...
- (void)MiXin_didServerResult_MiXinSucces:(MiXin_JXConnection *)task dict:(NSDictionary *)dict array:(NSArray *)array1{
...
#pragma mark - 请求出错回调
-(int) MiXin_didServerConnect_MiXinError:(MiXin_JXConnection*)aDownload error:(NSError *)error{
    [self MiXin_doSaveError];
    return hide_error;
}
...
-(void)MiXin_sendAlreadyRead_MiXinMsg{//发送“已读”消息
...
- (void)setRemindMsg:(MiXin_JXMessageObject *)msg {
    NSString *content;
...
//            content = [NSString stringWithFormat:@"%@%@",fromUserName,Localized(@"JXMessage_fileDownload")];
            break;
        case kRoomRemind_RoomName:
            content = [NSString stringWithFormat:@"%@%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_UpdateRoomName"),msg.content];
            break;
        case kRoomRemind_NickName:{
            content = [NSString stringWithFormat:@"%@%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_UpdateNickName"),msg.content];
        }
            break;
        case kRoomRemind_DelRoom:
        ...
        case kRoomRemind_AddMember:
            if([toUserId isEqualToString:fromUserId])
                content = [NSString stringWithFormat:@"%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_GroupChat")];
            else
                content = [NSString stringWithFormat:@"%@%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_InterFriend"),toUserName];
            break;
        case kLiveRemind_ExitRoom:
            if([toUserId isEqualToString:fromUserId]){
        ...
        case kRoomRemind_DelMember:
            if([toUserId isEqualToString:fromUserId]){
                content = [NSString stringWithFormat:@"%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_OutGroupChat")];
            }else{
                if ([toUserId isEqualToString:MY_USER_ID]) {
                    content = [NSString stringWithFormat:Localized(@"JX_OutOfTheGroup"),fromUserName];
                }else {
                    if ([fromUserName isEqualToString:toUserName]) {
                        content = [NSString stringWithFormat:@"%@%@",fromUserName,Localized(@"MiXin_JXRoomMember_MiXinVC_OutPutRoom")];
                    }else {
                        content = [NSString stringWithFormat:@"%@%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_KickOut"),toUserName];
                    }
                }
            }
...
//            content = [NSString stringWithFormat:@"%@%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_AddNewAdv"),msg.content];
            //处理公告消息类型
            NSString *newContent = [self getContentMsg:msg];
            content = [NSString stringWithFormat:@"%@%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_AddNewAdv"),newContent];
        ...
        case kRoomRemind_DisableSay:{
            if([msg.content longLongValue]==0){
                content = [NSString stringWithFormat:@"%@%@%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_Yes"),toUserName,Localized(@"MiXin_JXMessageObject_CancelGag")];
            }else{
                NSDate* d = [NSDate dateWithTimeIntervalSince1970:[msg.content longLongValue]];
                NSString* t = [TimeUtil formatDate:d format:@"MM-dd HH:mm"];
                content = [NSString stringWithFormat:@"%@%@%@%@%@",fromUserName,Localized(@"MiXin_JXMessageObject_Yes"),toUserName,Localized(@"MiXin_JXMessageObject_SetGagWithTime"),t];
                d = nil;
            }
            break;
        }
        ...
        case kRoomRemind_SetManage:{
            if ([msg.content integerValue] == 1) {
                content = [NSString stringWithFormat:@"%@%@%@%@",Localized(@"JXGroup_Owner"),Localized(@"MiXin_JXSetting_MinXinVC_Set"),toUserName,Localized(@"JXMessage_admin")];
            }else {
        ...
        case kRoomRemind_SetInvisible:{
            if ([msg.content integerValue] == 1) {
                content = [NSString stringWithFormat:@"%@%@%@%@",fromUserName,Localized(@"MiXin_JXSetting_MinXinVC_Set"),toUserName,Localized(@"JX_ForTheInvisibleMan")];
            }else if ([msg.content integerValue] == -1){
                content = [NSString stringWithFormat:@"%@%@%@",fromUserName,Localized(@"JX_EliminateTheInvisible"),toUserName];
            }else if ([msg.content integerValue] == 2){
                content = [NSString stringWithFormat:@"%@%@%@%@",fromUserName,Localized(@"MiXin_JXSetting_MinXinVC_Set"),toUserName,@"为监控人"];
            }else if ([msg.content integerValue] == 0){
...
{
    MiXin_JXMessageObject* newMsg = [[MiXin_JXMessageObject allocWithZone:zone] init];
...
- (MiXin_JXMessageObject *)getMsgWithMsgId:(NSString *)msgId {
    FMDatabase* db = [[JXXMPP sharedInstance] openUserDb:MY_USER_ID];
...
- (MiXin_JXMessageObject *)getMsgWithMsgId:(NSString *)msgId toUserId:(NSString *)toUserId {
    FMDatabase* db = [[JXXMPP sharedInstance] openUserDb:MY_USER_ID];
...
//处理公告消息类型
- (NSString *)getContentMsg:(MiXin_JXMessageObject *)msg{
    NSString *newContent = @"";
    //添加开启公告强提醒后数据格式判断
    //是否可以解析到字典
