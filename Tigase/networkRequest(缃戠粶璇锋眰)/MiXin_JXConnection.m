...
@implementation MiXin_JXConnection
...
-(void)dealloc{
//    NSLog(@"MiXin_JXConnection.dealloc:%@=%d",action,self.retainCount);
    self.action = nil;
    self.toView = nil;
//    self.userInfo = nil;
    self.delegate = nil;
...
- (NSString *)getSecret {
    ...
    if ([self.action isEqualToString:act_TransferWXPay] ||  [self.action isEqualToString:act_sendRedPacket] || [self.action isEqualToString:act_sendRedPacketV1] ||[self.action isEqualToString:act_OpenAuthInterface] || [self.action isEqualToString:act_alipayTransfer] || [self.action isEqualToString:act_sendTransfer] || [self.action isEqualToString:act_codePayment] || [self.action isEqualToString:act_codeReceipt] || [self.action isEqualToString:act_PayPasswordPayment] || [self.action isEqualToString:act_TransferToAdmin] || [self.action isEqualToString:act_otherSetInviteCode]) {
        return nil;
    }
