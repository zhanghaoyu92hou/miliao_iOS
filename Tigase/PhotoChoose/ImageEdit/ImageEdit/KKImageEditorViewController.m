...
#import "MiXin_JXCamera_MinXinVC.h"
...
//    [self.view setBackgroundColor:[KKImageEditorTheme theme].backgroundColor];
    [self.view setBackgroundColor:HEXCOLOR(0x000000)];
    
    self.title = self.toolInfo.title;
    [self initColorView]; //底部颜色
    [self initMenuView]; //底部视图
...
- (void)initColorView {
    if (_colorView) {
        _colorView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.height - JX_SCREEN_BOTTOM - 50, self.view.width, 50)];
        [_colorView setBackgroundColor:HEXCOLOR(0x000000)];
        [self.view addSubview:_colorView];
    }
}

#pragma -mark 底部工具view
- (void)initMenuView{
    if (_menuView == nil) {
...
//        _menuView.backgroundColor = [KKImageEditorTheme theme].toolbarColor;
        [_menuView setBackgroundColor:HEXCOLOR(0x000000)];
...
- (void)initNavigationBar{
    NSString *title;
    if ([self.delegate isKindOfClass:[MiXin_JXChat_MiXinViewController class]] || [self.delegate isKindOfClass:[MiXin_JXCamera_MinXinVC class]]) {
