...
@implementation MiXin_JXVideoPlayer
...
//        CGRect frame = THE_DEVICE_HAVE_HEAD ? CGRectMake(0, JX_SCREEN_TOP, JX_SCREEN_WIDTH, JX_SCREEN_HEIGHT-JX_SCREEN_TOP-JX_SCREEN_BOTTOM) : _firstBaseView.bounds;
        ...
        _wait = [[MiXin_JXWaitView alloc] initWithParent:_firstBaseView];
        [_wait start];
...
- (void)longPress:(UILongPressGestureRecognizer *)gestureRecognizer {
    MiXin_JXActionSheet_MiXinVC *actionVC = [[MiXin_JXActionSheet_MiXinVC alloc] initWithImages:@[] names:@[Localized(@"JX_SaveVideo")]];
...
- (void)dealloc {
    NSLog(@"MiXin_JXVideoPlayer.dealloc");
    self.parent = nil;
    self.videoFile = nil;
//    [_pauseBtn release];
...
//显示播放器
- (void)start{
    _player = [[MiXin_JXVideoPlayer_MiXinVC alloc] init];
    ...
    if (_isPreview) {  // 预览界面处理
        ...
        _timeLab.hidden = YES;
        _exitBtn.hidden = YES;
        _disBtn = [self MiXin_create_MiXinButtonWithFrame:CGRectMake(20, JX_SCREEN_HEIGHT-20-80, 80, 80) image:@"video_return" action:@selector(exitVideoPlayer)];
//        [_parent addSubview:_disBtn];
//        [_parent bringSubviewToFront:_disBtn];
        _sendBtn = [self MiXin_create_MiXinButtonWithFrame:CGRectMake(JX_SCREEN_WIDTH-20-80, JX_SCREEN_HEIGHT-20-80, 80, 80) image:@"video_gou" action:@selector(didButtonWithSendVideo)];
...
-(void)doPlayEnd{
    ...
    self.isPlaying = NO;
    if(self.delegate != nil && [self.delegate respondsToSelector:self.MiXin_didVideoPlayEnd])
        [self.delegate performSelectorOnMainThread:self.MiXin_didVideoPlayEnd withObject:self waitUntilDone:NO];
}
