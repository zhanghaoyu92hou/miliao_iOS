...
#import "MiXin_HBShowImageControl.h"
#import "NSStrUtil.h"
#import "UIImageView+HBHttpCache.h"
#import "ObjUrlData.h"
#import "JSONKit.h"
@implementation MiXin_HBShowImageControl
...
{
    UIImageView * imageView=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, (self.isCollect)?IMAGE_SIZE:MAX_WIDTH, (self.isCollect)?IMAGE_SIZE:MAX_HEIGHT)];
    [self addSubview:imageView];
...
-(void)lookImageAction:(UIGestureRecognizer*)sender
{
    ...
    [_imageList addTarget:self tapOnceAction:@selector(MiXin_dismissImageAction:)];
...
-(void)MiXin_dismissImageAction:(UIImageView*)sender
{
...
-(void)setImagesFileStr:(id)fileStr
{
    if(fileStr==nil)
        return;
...
+(float)heightForFileStr:(id)fileStr
{
    if(fileStr==nil)
        return 0;
    if ([fileStr isKindOfClass:[NSArray class]]) {
