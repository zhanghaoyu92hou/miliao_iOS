...
//        url.url= @"http://www.feizl.com/upload2007/2013_02/130227014423722.jpg";
        url.url= [g_server WH_getHeadImageOUrlWithUserId:userId];
        url.mime=@"image/pic";
        [smalls addObject:url];
    }
...
-(NSString*)getAllPraiseUsers{
    ...
    WH_JXUserObject *user = [[WH_JXUserObject sharedUserInstance] getFriendWithUserId:p.userId];
    s =  user.remarkName.length? user.remarkName : p.userNickName;
    
    for(int i=1;i<n;i++){
        p = [praises objectAtIndex:i];
        WH_JXUserObject *user = [[WH_JXUserObject sharedUserInstance] getFriendWithUserId:p.userId];
        s = [s stringByAppendingString:@"，"];
        s = [s stringByAppendingString:user.remarkName.length? user.remarkName : p.userNickName];
    }
//    if(m>=20)
