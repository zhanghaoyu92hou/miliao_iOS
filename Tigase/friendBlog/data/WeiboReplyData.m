...
//        CFRelease(sfontRef);
 ...
 //   }
    UIFont *font = _font?:sysFontWithSize(14);
    UIColor *textColor = _textColor?:HEXCOLOR(0x576b95);
//    CTFontRef fontRef=CTFontCreateWithName((__bridge CFStringRef)(font.fontName),font.pointSize,NULL);
    NSDictionary *attDic = @{NSForegroundColorAttributeName:textColor,NSFontAttributeName:font};
    NSString * s = nil;
    if (toNickName) {
        s = [NSString stringWithFormat:@"%@%@%@",userNickName,Localized(@"JX_Reply"),toNickName];
        NSMutableAttributedString * attributedString = [[NSMutableAttributedString alloc]initWithString:s attributes:attDic];
//        [attributedString addAttribute:NSForegroundColorAttributeName value:textColor range:NSMakeRange(0, [userNickName length])];
//        [attributedString addAttribute:NSForegroundColorAttributeName value:textColor range:NSMakeRange([userNickName length] +2, [toNickName length])];
...
//        NSMutableAttributedString * strings=[[NSMutableAttributedString alloc]initWithString:s attributes:[NSDictionary dictionaryWithObjectsAndKeys:(__bridge id)fontRef,kCTFontAttributeName,textColor.CGColor,kCTForegroundColorAttributeName,nil]];
        self.title= [[NSMutableAttributedString alloc]initWithString:s attributes:attDic];
    }
...
-(MatchParser*)createMatch:(float)width
{
    if(_match==nil||![_match isKindOfClass:[MatchParser class]]){
        ...
        parser.font=_font?:sysFontWithSize(14);
        if (_textColor) {
            parser.textColor = _textColor;
        }
        parser.width=width;
...
-(void)getHeight2{
    if(height2>0)
        return;
    height2 = 15;
    WH_JXEmoji* p = [[WH_JXEmoji alloc]initWithFrame:CGRectMake(0, 0, 220, 15)];
