...
#import "WH_AnimOperation.h"

@interface WH_AnimOperationManager ()
/// 队列1
...
@implementation WH_AnimOperationManager

+ (instancetype)sharedManager
{
    static WH_AnimOperationManager *manager;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[WH_AnimOperationManager alloc] init];
...
- (void)animWithUserID:(NSString *)userID model:(GiftModel *)model finishedBlock:(void(^)(BOOL result))finishedBlock {
    ...
    NSInteger giftCount = model.giftCount;
    // 在有用户礼物信息时
    if ([self.userGigtInfos objectForKey:userID]) {
        // 如果有操作缓存，则直接累加，不需要重新创建op
        if ([self.operationCache objectForKey:userID]!=nil) {
            WH_AnimOperation *op = [self.operationCache objectForKey:userID];
            op.presentView.giftCount = giftCount;
            [op.presentView shakeNumberLabel];
            return;
        }
         // 没有操作缓存，创建op
        WH_AnimOperation *op = [WH_AnimOperation WH_AnimOperationWithUserID:userID model:model finishedBlock:^(BOOL result,NSInteger finishCount) {
            // 回调
            if (finishedBlock) {
