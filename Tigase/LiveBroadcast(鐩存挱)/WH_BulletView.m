...
- (instancetype)initWithCommentDic:(BulletSettingDic *)commentDic
{
    ...
    if (self)
    {
        if ([commentDic bulletText])
        {
            ...
            //默认时间
            _wh_moveDuration = [commentDic bulletAnimationDuration];
            
            //计算速度
            _screenWidth = [UIScreen mainScreen].bounds.size.width;
            _wh_speed = (CGRectGetWidth(self.frame) + _screenWidth)/_wh_moveDuration;
            _real_speed = _wh_speed*[commentDic bulletAnimationSpeedRate];
        }
    }
    return self;
}
- (void)reloadDataWithDic:(BulletSettingDic *)reloadDic
{
    if ([reloadDic bulletText])
    {
        ...
        _screenWidth = [UIScreen mainScreen].bounds.size.width;
        _wh_speed = (CGRectGetWidth(self.frame) + _screenWidth)/_wh_moveDuration;
        _real_speed = _wh_speed*[reloadDic bulletAnimationSpeedRate];
    }
}
//开始动画
- (void)startAnimation
{
    ...
    [UIView animateWithDuration:dur delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
    ...
    } completion:^(BOOL finished)
     {
         ...
         //弹幕开始进入屏幕
         if (weakSelf.wh_moveBlock)
             weakSelf.wh_moveBlock(MoveIn);
