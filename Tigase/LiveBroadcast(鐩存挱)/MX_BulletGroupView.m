...
#import "MiXinBulletGroupView.h"

#define PROLOADNUM 15

@interface MiXinBulletGroupView ()
{
...
@implementation MiXinBulletGroupView
...
-(void)loadButtetView
{
    ...
    for (int i = j; i > 0; i--)
    {
        ...
        if (comment)
        {
            ...
            [self createMiXinBulletView:comment trajectory:trajectory];
        }
        else
        {
            //没有评论时
...
- (void)createMiXinBulletView:(NSString *)comment trajectory:(int)trajectory
{
    if (self.isStopped)return;
    
    //创建MiXinBulletView，如果有移除来的，那么重用；
    MiXinBulletView *view;
    ...
    {
        view = [[MiXinBulletView alloc] initWithCommentDic:_bulletDic];
    }
    view.trajectory = trajectory;
    [_bulletArray addObject:view];
    
    __unsafe_unretained MiXinBulletView *weakMiXinBulletView = view;
    __unsafe_unretained MiXinBulletGroupView *weakSelf = self;
    ...
    view.moveBlock = ^(CommentMoveStatus status)
    {
        ...
        switch (status) {
            ...
            case Enter: {
                ...
                if (comment)
                {
                    [weakSelf createMiXinBulletView:comment trajectory:trajectory];
                } else
                {
                    ...
                    if (moreComment)
                    {
                        [weakSelf createMiXinBulletView:moreComment trajectory:trajectory];
                    }else
                    {
                        //可用的没了，要保存下顺序数组
            ...
            case MoveOut:
            {
                ...
                if ([weakSelf.bulletArray containsObject:weakMiXinBulletView])
                {
                    [weakSelf.bulletArray removeObject:weakMiXinBulletView];
                    [weakSelf.reuseableBulletArray addObject:weakMiXinBulletView];
...
//这个字典中设置弹幕的字体和颜色，具体哪些键，请参看MiXinBulletView.h中的键定义。
