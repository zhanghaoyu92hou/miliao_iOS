...
@implementation WH_JXTopSiftJobView
...
- (void)setWh_dataArray:(NSArray *)dataArray {
    if(_wh_dataArray != dataArray){
//        [_dataArray release];
        _wh_dataArray = dataArray;
        _btnArray = [[NSMutableArray alloc] init];
        
        if (self.wh_isShowMoreParaBtn) {
            //更多筛选条件按钮
...
//            [leftVerticalLine release];
        ...
        self.itemBtnWidth = 0;
        if (self.wh_isShowMoreParaBtn) {
            self.itemBtnWidth = (self.frame.size.width-50)/self.wh_dataArray.count;
        }else {
            self.itemBtnWidth = self.frame.size.width / self.wh_dataArray.count;
        }
        
        [_btnArray removeAllObjects];
        for (int i=0; i<_wh_dataArray.count; i++) {
            UIButton *itemBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.itemBtnWidth*i, 0, self.itemBtnWidth, ITEMBTN_HEIGHT)];
            itemBtn.tag = i+100;
            [itemBtn setTitle:_wh_dataArray[i] forState:UIControlStateNormal];
            itemBtn.titleLabel.font = sysFontWithSize(16);
...
//            [itemBtn release];
        ...
        [self WH_resetBottomLineIndex:_wh_preferred];
        
    }
}
-(void)setWh_preferred:(NSUInteger)preferred{
    if(_wh_preferred != preferred){
        _wh_preferred = preferred;
    }
}

- (void)WH_moveBottomSlideLine:(CGFloat)offsetX{
...
- (void)itemBtnClick:(UIButton *)btn{
    if (btn.selected) {  //重复点击按钮
        ...
        [self.wh_delegate performSelector:@selector(topItemBtnClick:) withObject:btn];
    }
...
- (void)paraItemBtnClick:(UIButton *)btn{
    ...
    [self.wh_delegate performSelector:@selector(paraItemBtnClick:) withObject:btn];
}
...
- (void)WH_resetItemBtnWith:(CGFloat)offsetX{
    for (UIView *view in self.subviews) {
        if (view.tag >= 100 && view.tag <= 102) {
...
//重置选中的参数按钮状态
- (void)WH_resetSelParaBtnTransform{
...
//选中某个筛选条件后改变按钮文案
- (void)WH_resetWithIndex:(NSInteger)index itemId:(int)itemId itemValue:(NSString *)value{
    for (UIView *view in _moreParaView.subviews) {
        if (view.tag == index) {
            UIButton *btn = (UIButton *)view;
            if (itemId == 0) {
                [btn setTitle:Localized(@"WaHu_JXTopSiftJobView_Default") forState:UIControlStateNormal];
                btn.selected = NO;
            }else{
...
//选中薪水筛选条件后改变按钮文案
- (void)WH_resetWithIndex:(NSInteger)index min:(NSInteger)min max:(NSInteger)max{
    for (UIView *view in _moreParaView.subviews) {
        if (view.tag == index) {
            UIButton *btn = (UIButton *)view;
            if (min == 0 && max == 0) {
                [btn setTitle:Localized(@"WaHu_JXTopSiftJobView_FaceToFace") forState:UIControlStateNormal];
                btn.selected = NO;
            }else{
