...
@implementation WH_menuImageView
...
//        self.showSelected = YES;
        ...
        if(menuFont==nil)
            self.menuFont = sysFontWithSize(13);
        [self draw];
    }
    return self;
}
...
-(void)draw{
    ...
    btn = [self WH_create_WHButtonWithRect:CGRectMake(n, t, width, 30)
...
//    [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    ...
    for(i=1;i<[items count]-1;i++){
        btn = [self WH_create_WHButtonWithRect:CGRectMake(n+width*i, t, width, 30)
...
//    [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    ...
    if ([title isEqualToString:Localized(@"JX_MyRoom")]||[title isEqualToString:Localized(@"JX_AllRoom")]) {
        [button setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
        [button.titleLabel setFont:sysFontWithSize(16)];
    }
