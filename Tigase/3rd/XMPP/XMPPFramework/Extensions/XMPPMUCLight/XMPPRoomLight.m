...
- (void)createRoomLightWithMembersJID:(nullable NSArray<XMPPJID *> *) members{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		[configuration WH_addChild:[NSXMLElement elementWithName:@"roomname" stringValue:roomname]];
		...
		for (XMPPJID *jid in members){
			...
			[occupants WH_addChild:userElement];
		}
		
		[query WH_addChild:configuration];
		[query WH_addChild:occupants];
...
- (void)fetchMembersList{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		[query WH_addChild:[NSXMLElement elementWithName:@"version" stringValue:self.memberListVersion]];
		[iq WH_addChild:query];
...
- (void)getConfiguration {
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		[query WH_addChild:[NSXMLElement elementWithName:@"version" stringValue:self.configVersion]];
		[iq WH_addChild:query];
