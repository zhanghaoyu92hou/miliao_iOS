...
- (void)updateSharedStatus:(NSString *)status show:(NSString *)show invisible:(BOOL)invisible {
	...
	[statusIQ WH_addChild:[self packSharedStatus:sharedStatusUpdate]];
...
- (void)discoverCapabilities {
	...
	[discoIQ WH_addChild:[XMPPElement elementWithName:@"query" xmlns:GOOGLE_DISCO_INFO]];
...
- (XMPPElement *)packSharedStatus:(NSDictionary *)sharedStatus {
	...
	[element WH_addChild:[XMPPElement elementWithName:XMPPGoogleSharedStatusStatus
                                     stringValue:sharedStatus[XMPPGoogleSharedStatusStatus]]];
	[element WH_addChild:[XMPPElement elementWithName:XMPPGoogleSharedStatusShow
		...
		if([key isEqualToString:XMPPGoogleSharedStatusShowAvailable] ||
		   [key isEqualToString:XMPPGoogleSharedStatusShowBusy]) {
			...
			for(NSString *status in statusList)
				[statusElement WH_addChild:[XMPPElement elementWithName:@"status" stringValue:status]];
			[element WH_addChild:statusElement];
...
- (void)setStatusAvailability:(NSString *)statusAvailability {
    ...
    [presence WH_addChild:[XMPPElement elementWithName:@"priority" stringValue:GOOGLE_PRESENCE_PRIORITY]];
	...
	if(self.statusAvailability) {
		...
		[presence WH_addChild:show];
	}
	
	if(self.statusMessage) {
		[status setStringValue:self.statusMessage];
		[presence WH_addChild:status];
	}
