...
- (void)requestSlotFromService:(XMPPJID*)serviceJID
	...
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        if (filename) {
            [request WH_addChild:[XMPPElement elementWithName:@"filename" stringValue:filename]];
        }
		[request WH_addChild:[XMPPElement elementWithName:@"size" numberValue:[NSNumber numberWithUnsignedInteger:size]]];
        if (contentType) {
            [request WH_addChild:[XMPPElement elementWithName:@"content-type" stringValue:contentType]];
        }
		
		[iq WH_addChild:request];
