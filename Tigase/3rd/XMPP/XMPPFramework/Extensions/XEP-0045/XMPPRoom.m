...
- (void)joinRoomUsingNickname:(NSString *)desiredNickname history:(NSXMLElement *)history password:(NSString *)passwd
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (history)
		{
			[x WH_addChild:history];
		}
		if (passwd)
		{
			[x WH_addChild:[NSXMLElement elementWithName:@"password" stringValue:passwd]];
		}
		
		XMPPPresence *presence = [XMPPPresence presenceWithType:nil to:myRoomJID];
		[presence WH_addChild:x];
...
- (NSXMLElement *)inviteElementWithJid:(XMPPJID *)jid invitationMessage:(NSString *)invitationMessage {
	...
	if ([invitationMessage length] > 0) {
		[invite WH_addChild:[NSXMLElement elementWithName:@"reason" stringValue:invitationMessage]];
	}
	return invite;
}
