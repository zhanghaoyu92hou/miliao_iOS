...
- (void)addOutOfBandURL:(NSURL *)URL desc:(NSString *)desc
{
	...
	if([[URL path] length])
	{
		...
		[outOfBand WH_addChild:URLElement];
	}
	
	if([desc length])
	{
		NSXMLElement *descElement = [NSXMLElement elementWithName:@"desc" stringValue:desc];
		[outOfBand WH_addChild:descElement];
	}
	
	[self WH_addChild:outOfBand];
}
...
- (XMPPIQ *)generateOutOfBandDataFailureResponse
{
	...
	[errorElement WH_addChild:itemNotFoundElement];
	
	[outOfBandDataFailureResponse WH_addChild:errorElement];
...
- (XMPPIQ *)generateOutOfBandDataRejectResponse
{
	...
	[errorElement WH_addChild:notAcceptableElement];
	
	[outOfBandDataRejectResponse WH_addChild:errorElement];
