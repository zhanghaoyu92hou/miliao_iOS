...
+(XMPPIQ *)rpcTo:(XMPPJID *)jid methodName:(NSString *)method parameters:(NSArray *)parameters {
	...
	for (id parameter in parameters) {
		[params WH_addChild:[self paramElementFromObject:parameter]];
	}
	
	[methodCall WH_addChild:methodName];
	[methodCall WH_addChild:params];
	[jabberRPC WH_addChild:methodCall];
	[iq WH_addChild:jabberRPC];
	return iq;
}
...
+(NSXMLElement *)valueElementFromArray:(NSArray *)array {
	...
	for (id object in array) {
		[data WH_addChild:[self valueElementFromObject:object]];
	}
...
+(NSXMLElement *)valueElementFromDictionary:(NSDictionary *)dictionary {
	...
	for (NSString *key in dictionary) {
		...
		[member WH_addChild:name];
		[member WH_addChild:[self valueElementFromObject:dictionary[key]]];
	}
