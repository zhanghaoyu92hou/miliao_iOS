...
+ (instancetype)enableNotificationsElementWithJID:(XMPPJID *)jid node:(NSString *)node options:(nullable NSDictionary<NSString *,NSString *> *)options elementId:(NSString *)elementId
{
    ...
    if ([options count]) {
        ...
        [formTypeField WH_addChild:[NSXMLElement elementWithName:@"value" stringValue:@"http://jabber.org/protocol/pubsub#publish-options"]];
        [dataForm WH_addChild:formTypeField];
        ...
        [options enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSString * _Nonnull obj, BOOL * _Nonnull stop) {
            ...
            [formField WH_addChild:[NSXMLElement elementWithName:@"value" stringValue:obj]];
            [dataForm WH_addChild:formField];
        }];
        [enableElement WH_addChild:dataForm];
    }
