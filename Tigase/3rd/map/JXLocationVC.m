...
- (id)init
{
    self = [super init];
    if (self) {
        self.title = Localized(@"WaHu_JXUserInfo_WaHuVC_Loation");
        self.wh_heightHeader = _locationType ==JXLocationTypeShowStaticLocation ? 0 : JX_SCREEN_TOP;
...
- (void)viewDidLoad{
    ...
    [self customView];
    self.wh_tableBody.contentSize = CGSizeMake(0, JX_SCREEN_HEIGHT - JX_SCREEN_TOP);
...
- (void)customView{
    if (_locationType == JXLocationTypeShowStaticLocation) {
        ...
        UIButton* btn = [UIFactory WH_create_WHButtonWithImage:@"adress_navigation" highlight:nil target:self selector:@selector(moreBtnAction)];
        ...
        //发送
        if(self.isSend){
            _sendButton = [UIFactory WH_create_WHCommonButton:Localized(@"JX_Send") target:self action:@selector(onSelect)];
            _sendButton.frame = CGRectMake(JX_SCREEN_WIDTH - 60, JX_SCREEN_TOP - 34, 60, 24);
            [self.wh_tableHeader addSubview:_sendButton];
...
- (void)initBaiduMapView{
    _baiduMapView = [[BMKMapView alloc]initWithFrame:CGRectMake(0, 0, self.wh_tableBody.frame.size.width, self.wh_tableBody.frame.size.height)];
    _baiduMapView.zoomLevel = 13;
    _baiduMapView.minZoomLevel = 6;

    _baiduMapView.showsUserLocation = YES;
    [self.wh_tableBody addSubview:_baiduMapView];
}
...
- (void)mapView:(BMKMapView *)mapView regionDidChangeAnimated:(BOOL)animated
{
    if (_locationType == JXLocationTypeShowStaticLocation) {
        ...
        NSLog(@"精度%f 维度%f   %d",self.latitude,self.longitude,self.wh_tableBody.userInteractionEnabled);
//        CLLocationCoordinate2D  coor;
...
-(NSString *)screenShotAction{
    ...
    [g_server WH_saveImageToFileWithImage:logoImage file:filePath isOriginal:NO];
...
//    self.textString = model.address;
    ...
    //定位到某个指定地点
    JXMapData * staticData = [JXMapData modelWithJXPlaceMarkModel:model];
    [self makeMapCenter:staticData];
    [self addPointAnnotations:_locations];
    self.title = staticData.title;
}
