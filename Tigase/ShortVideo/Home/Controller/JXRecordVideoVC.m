...
#import "GPUImage.h"
#import "MiXin_GPUImageBeautifyFilter.h"
#import "MiXin_JXVideoPlayer.h"
#import "UIImage+Color.h"
#import "JXSelectMusicVC.h"
#import "EditAudioVideo.h"
#import "addMsgVC.h"
#import "MiXin_JXConvertMedia.h"
#import "JXSelectMusicModel.h"
#import "UIButton+WH_Button.h"

#define kCameraVideoPath [FileInfo getUUIDFileName:@"mp4"]

@interface JXRecordVideoVC ()<JXSelectMusicVCDelegate>
...
@property (nonatomic, strong) MiXin_GPUImageBeautifyFilter *beautifyFilter;
...
@property (nonatomic, strong) MiXin_JXVideoPlayer *player;
...
@implementation JXRecordVideoVC
...
//    [switchBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    ...
    _beautifyFilter = [[MiXin_GPUImageBeautifyFilter alloc] init];
...
//    [self addPhotoStyle:primaryView];
    ...
    UIButton *cancelImageBtn = [self MiXin_create_MiXinButtonWithFrame:CGRectMake(20, JX_SCREEN_HEIGHT-20-80, 80, 80)  image:@"video_return" action:@selector(cancelImageBtnAction:)];
    [_iv addSubview:cancelImageBtn];
    UIButton *confirmBtn = [self MiXin_create_MiXinButtonWithFrame:CGRectMake(JX_SCREEN_WIDTH-20-80, JX_SCREEN_HEIGHT-20-80, 80, 80) image:@"video_gou" action:@selector(MiXin_confirmBtnAction:)];
...
// 确定此照片
- (void) MiXin_confirmBtnAction:(UIButton *)btn {
...
- (void) addPhotoStyle:(UIView *)parentView{
    ...
    //                       ];
    //美颜
    MiXin_GPUImageBeautifyFilter *BeautifyFilter = [[MiXin_GPUImageBeautifyFilter alloc] init];
    //哈哈镜效果
...
- (UIButton *)MiXin_create_MiXinButtonWithFrame:(CGRect)frame image:(NSString *)image action:(SEL)action {
...
//    [_beautifyFilter removeAllTargets];
//
//    _beautifyFilter = [MiXin_GPUImageBeautifyFilter alloc];
//
...
//    [self.view addSubview:_timeLabel];
    ...
    btn.titleLabel.font = g_factory.font13;
...
//    _selectMusic.font = g_factory.font13;
...
- (void)selectMusicVC:(JXSelectMusicVC *)vc selectMusic:(JXSelectMusicModel *)model{
    ...
    _audioPlayer = [[JXAudioPlayer alloc]initWithParent:self.view frame:CGRectNull isLeft:YES];
...
- (void) showPreview:(NSString *)path{
    ...
    _player= [MiXin_JXVideoPlayer alloc];
...
- (void)synthesizeClick:(NSString *)videoPath AudioPath:(NSString *)audioPath {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [EditAudioVideo editVideoSynthesizeVieoPath:[NSURL fileURLWithPath:videoPath] BGMPath:[NSURL fileURLWithPath:audioPath] needOriginalVoice:NO videoVolume:0 BGMVolume:1 complition:^(NSURL *outputPath, BOOL isSucceed) {
...
- (void)didSendBtn:(UIButton *)btn {
    ...
    vc.videoFile = self.outputFileName;
    vc.isShortVideo = YES;
...
//    [parentLayer addSublayer:aLayer];
    ...
    [_exporter exportAsynchronouslyWithCompletionHandler:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (_audioPlayer.audioFile.length > 0) {
                [self synthesizeClick:self.outputFileName AudioPath:_audioPlayer.audioFile];
            }else {
