...
- (void)customView {
    ...
    [confirmBtn addTarget:self action:@selector(WH_confirmBtnAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)WH_confirmBtnAction {
    ...
    if ([self.delegate respondsToSelector:@selector(webAuthViewWH_confirmBtnAction)]) {
        [self.delegate webAuthViewWH_confirmBtnAction];
    }
